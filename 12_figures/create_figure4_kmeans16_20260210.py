...
def create_figure(matrix, mean_shap, cluster_assignments, cluster_boundaries,
    ...
    gs = gridspec.GridSpec(6, 5, figure=fig,
                           ...
                           width_ratios=[0.18, 0.08, 0.03, 1.0, 0.58],
                           left=0.01, right=0.99, top=0.98, bottom=0.06,
    ...
    for (start, end, cl) in col_boundaries:
        ...
        rect = Rectangle((start, 0), width, 1, facecolor=color,
                          edgecolor='white', linewidth=0.25, alpha=0.3)
    ...
    ax_shap_bar.set_xticklabels(['0', f'{shap_vals.max():.4f}'], fontsize=6)
    ax_shap_bar.tick_params(axis='x', which='both', top=True, bottom=False,
                            width=0.25, length=2, pad=1)
    ax_shap_bar.set_yticks([])
    ax_shap_bar.set_xlabel('Mean |SHAP|', fontsize=6)
    ...
    for idx, (start, end, c) in enumerate(cluster_boundaries):
        ...
        for i in range(0, len(top_labels), line_size):
            line = ', '.join(top_labels[i:i + line_size])
            if len(line) > max_line_chars:
                line = line[:max_line_chars - 1] + '…'
            pfam_lines.append(line)
        pfam_str = '\n'.join(pfam_lines)
        ...
        ax_annot.text(0.02, y_center, header,
                      fontsize=6, va='bottom', color='#000000',
                      fontweight='bold', clip_on=False)
        ax_annot.text(0.02, y_center, pfam_str,
                      fontsize=6, va='top', color='#000000',
    ...
    ax_corr.set_ylabel('|r| top GEE', fontsize=6)
    ax_corr.tick_params(axis='y', labelsize=6, width=0.25, length=2)
    ...
    ax_corr.set_xticklabels(gee_labels, rotation=90, fontsize=6, ha='center')
    ax_corr.tick_params(axis='x', width=0.25, length=2, pad=1)
    ...
    cbar.set_label('Z-score (row-normalized SHAP)', fontsize=6)
...
def main():
    ...
    # Save
    for fmt in ['pdf', 'svg']:
        out_path = f'{OUT_DIR}/Figure4_kmeans16_{TIMESTAMP}.{fmt}'
        fig.savefig(out_path, format=fmt, dpi=600,
                    transparent=True, edgecolor='none')
        print(f"Saved: {out_path}")
        # Also save as canonical name
        canonical = f'{OUT_DIR}/Figure4_aef_bicluster.{fmt}'
        fig.savefig(canonical, format=fmt, dpi=600,
                    transparent=True, edgecolor='none')
        print(f"Saved canonical: {canonical}")

    # Provenance
    prov_path = f'{OUT_DIR}/Figure4_kmeans16_{TIMESTAMP}_provenance.txt'
