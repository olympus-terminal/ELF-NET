...
fig = plt.figure(figsize=(7.0, 4.0), layout=None)
...
for i, target in enumerate(available_targets[:6]):
    ...
    ax.set_title(f'{name} ({target.split(".")[0]})')
...
ax_hist.text(0.97, 0.92, f'n = {len(r2_values):,} domains\n'
             ...
             transform=ax_hist.transAxes, fontsize=6, verticalalignment='top',
...
# ── Save ──
for fmt in ['pdf', 'svg']:
    outpath = os.path.join(OUTDIR, f'Figure5_forward_calibration_{TIMESTAMP}.{fmt}')
