#!/usr/bin/env python3
"""
Regenerate FTR1 (PF03239) forward model predictions using the SAME methodology
as Figure 5 panels A-B, D-F:
  - Input: algagpt_gee_pfam_merged_GPS_RECOVERED_20260124_114445.tsv (satellite-only)
  - CV: 10-fold spatial block (2° grid cells)
  - XGBoost: n_estimators=200, max_depth=6, lr=0.1, subsample=0.8, etc.
  - CLR: pseudocount=0.5, prevalence >= 5%
  - Satellite-only env features (no WOA23 nutrients)

This makes FTR1 directly comparable to the other 5 calibration panels.

Output: MANUSCRIPT/source_data/panel_regen/panel_FTR1_satellite_obs_pred_TIMESTAMP.tsv
"""
import os
import sys
import datetime
...
MERGED_PATH = BASE_DIR / '03_analyses/ALGAGPT-based-analyses/algagpt_gee_pfam_merged_GPS_RECOVERED_20260124_114445.tsv'
OUT_DIR = BASE_DIR / 'MANUSCRIPT/source_data/panel_regen'
OUT_DIR.mkdir(parents=True, exist_ok=True)
TIMESTAMP = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')

TARGET_PFAM = 'PF03239'  # FTR1 iron permease — may appear as PF03239.XX
...
# Find FTR1 column (may have version suffix)
ftr1_col = None
for c in PFAM_COLS:
    if c.startswith(TARGET_PFAM):
        ftr1_col = c
        break
if ftr1_col is None:
    print(f"ERROR: {TARGET_PFAM} not found in PFAM columns"); sys.exit(1)
print(f"FTR1 column: {ftr1_col}")
...
if ftr1_col not in pfam_col_to_idx:
    print(f"ERROR: {ftr1_col} filtered out (prevalence < {PREVALENCE_THRESHOLD})")
    prev_val = prevalence[PFAM_COLS.index(ftr1_col)]
    print(f"  FTR1 prevalence: {prev_val:.4f}")
    sys.exit(1)

col_idx = pfam_col_to_idx[ftr1_col]
y_clr = X_pfam_clr[:, col_idx]

X_env_raw = df_gps[available_env].apply(pd.to_numeric, errors='coerce').values

print(f"\n=== Running FTR1 10-fold spatial block CV ===")
...
for fold_idx in range(N_FOLDS):
    ...
    print(f"  Fold {fold_idx}: R²={r2_fold:.4f}, n_test={len(test_idx)}")
...
print(f"  Fold R² mean = {np.mean(fold_vals):.4f} ± {np.std(fold_vals):.4f}")
print(f"  n = {has_pred.sum()}")

# Save per-sample predictions
out_tsv = OUT_DIR / f'panel_FTR1_satellite_obs_pred_{TIMESTAMP}.tsv'
out_df = pd.DataFrame({
    'assembly_id': df_gps.loc[has_pred, 'assembly_id'].values,
    'observed_clr': y_clr[has_pred],
    'predicted_clr': y_pred_all[has_pred],
})
with open(out_tsv, 'w') as f:
    ...
    f.write(f"#   CV: 10-fold spatial block (2° grid), satellite-only features\n")
    f.write(f"#   XGB: n_est=200, depth=6, lr=0.1, subsample=0.8\n")
    f.write(f"#   CLR: pseudocount=0.5, prevalence>=5%\n")
    f.write(f"#   Target: {ftr1_col} (FTR1 iron permease)\n")
    f.write(f"#   Overall R² = {overall_r2:.4f}\n")
    f.write(f"#   Fold R² = {np.mean(fold_vals):.4f} ± {np.std(fold_vals):.4f}\n")
    f.write(f"#   n = {has_pred.sum()}\n")
